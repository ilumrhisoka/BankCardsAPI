...
*   **Попытка пользователя выполнить действие администратора:**
    *   **Эндпоинт:** `DELETE /api/admin/users/{userId}` (используя токен `john.doe`, а не `adminUser`)
    *   **Ожидаемый результат:** `HTTP 403 Forbidden` с соответствующим `ErrorResponse`.
*   **Попытка перевода с недостаточным балансом:**
    *   **Эндпоинт:** `POST /api/user/transfers` (используя токен `john.doe`)
    *   **Тело запроса:** Попытайтесь перевести сумму, превышающую текущий баланс `fromCardId`.
    *   **Ожидаемый результат:** `HTTP 400 Bad Request` с сообщением об `InsufficientFundsException` в `ErrorResponse`.
*   **Попытка доступа к несуществующему ресурсу (например, карте):**
    *   **Эндпоинт:** `GET /api/user/cards/9999` (используя токен `john.doe`)
    *   **Ожидаемый результат:** `HTTP 404 Not Found` с сообщением об `CardNotFoundException` в `ErrorResponse`.
*   **Попытка регистрации с уже существующим именем пользователя:**
    *   **Эндпоинт:** `POST /api/auth/register`
    *   **Тело запроса:** Повторите регистрацию пользователя `john.doe`.
    *   **Ожидаемый результат:** `HTTP 409 Conflict` с сообщением о `DuplicateUsernameException` в `ErrorResponse`.